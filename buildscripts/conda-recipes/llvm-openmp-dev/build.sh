...
if [ "${BUILD_STANDALONE}" = "1" ]; then
  rm -rf openmp-"${PACKAGE_VERSION}".src*
  curl -L https://github.com/llvm/llvm-project/releases/download/llvmorg-"${PACKAGE_VERSION}"/openmp-"${PACKAGE_VERSION}".src.tar.xz -o openmp-"${PACKAGE_VERSION}".src.tar.xz
  EXPECTED_HASH="4f731ff202add030d9d68d4c6daabd91d3aeed9812e6a5b4968815cfdff0eb1f"
  # Compute the actual hash
  ACTUAL_HASH=$(sha256sum openmp-"${PACKAGE_VERSION}".src.tar.xz | awk '{print $1}')
  if [ "$ACTUAL_HASH" != "$EXPECTED_HASH" ]; then
    echo "SHA256 checksum does not match! Exiting."
    exit 1
  fi

  tar xf openmp-"${PACKAGE_VERSION}".src.tar.xz
  patch -p1 < patches/*
